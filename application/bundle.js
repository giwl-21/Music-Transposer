(function () { function r(e, n, t) { function o(i, f) { if (!n[i]) { if (!e[i]) { var c = "function" == typeof require && require; if (!f && c) return c(i, !0); if (u) return u(i, !0); var a = new Error("Cannot find module '" + i + "'"); throw a.code = "MODULE_NOT_FOUND", a } var p = n[i] = { exports: {} }; e[i][0].call(p.exports, function (r) { var n = e[i][1][r]; return o(n || r) }, p, p.exports, r, e, n, t) } return n[i].exports } for (var u = "function" == typeof require && require, i = 0; i < t.length; i++)o(t[i]); return o } return r })()({
  1: [function (require, module, exports) {
    //Application converts notes to MIDI, March 22, 2019
    var MidiWriter = require('midi-writer-js');

    window.outputToMidi = function(output, fRate, signal, sRate) {
      //Signal: for beat detection, from getPeaks
      //signal rate: for tempo temporal conversion
      //Output: parsing
      //Rate: Framerate for correct temporal conversion
      var allNotes = [];
      for (o in output[0]) {
        var onNote = false;
        for (k in output) {
          //If there is a sound after nothing, create a new note and add to allNotes
          if (!onNote) {
            if (output[k][o] == 1) {
              allNotes.push(new Note(o + 12, k, 1));//cos midi is 12 valuse above our value from musicXML parser
              onNote = true;
            }
          }
          else {
            if (output[k][o] == 1) {
              allNotes[allNotes.length - 1].duration++;
            }
            else {
              onNote = false;
            }
      ...
      //getting lengths
      var scanRange = 5;
      var beatLengthArray = findBeatLengths(signal, sRate, scanRange);
      console.log(allNotes);
      console.log(beatLengthArray);


      //WRITE MIDI findBeatLengths
      track = new MidiWriter.Track();
      track.addTrackName("Your Song!")
      for (note in allNotes) {
        var beatLength = beatLengthArray[Math.floor(allNotes[note].index / allNotes.length * beatLengthArray.length)]; /*tempo ish*/
        var beat = (allNotes[note].duration / fRate) / beatLength;
        var location = (allNotes[note].index / fRate) / beatLength;
        track.addEvent(new MidiWriter.NoteEvent({
          pitch: allNotes[note].pitch,
          duration: 'T' + Math.floor(beat * 120),//120 per sixteent h note
          startTick: Math.floor(location * 120)
        }));
      }
    ...
    function Note(pitch, startIndex, duration) {
      this.pitch = pitch + 12;//because midi is our deteced note + 12
      this.index = startIndex;
      this.duration = duration; //in frames, converted later
    }

  }, { "midi-writer-js": 7 }], 2: [function (require, module, exports) {
    'use strict'
  ...
  }, { "../package.json": 8, "_process": 6, "buffer": 4, "fs": 3, "tonal-midi": 10 }], 8: [function (require, module, exports) {
    module.exports = {
